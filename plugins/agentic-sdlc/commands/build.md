...
Invariant 1 removes the first half outright. A story branches from the epic tip
...
**Merge `origin/<base>` in before opening the PR, and again before asking for
review.**

```bash
git merge origin/<base>   # then run the four gates before pushing
```
...
full gate**: `pnpm typecheck && pnpm lint && pnpm test && pnpm build`. Tests alone
are not enough: a resolution that drops a closing brace from a type file still
passes every test and fails only at `build`, with the error reported at the *next*
